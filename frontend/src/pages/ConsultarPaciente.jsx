// src/pages/ConsultarPaciente.jsx
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import DashboardLayout from '../layouts/DashboardLayout';
import { Search, UserPlus, FolderOpen, MapPin, User, ShieldAlert } from 'lucide-react';
...
export default function ConsultarPaciente() {
    ...
    async function buscarPacientes() {
        ...
        try {
            ...
            // Alinhado com o seu back-end: se houver termo de busca, envia como query param, senão traz todos
            const url = pesquisa
                ? `http://localhost:3000/funcionarios?busca=${pesquisa}`
            ...
            setErro('Erro ao conectar com a API. Certifique-se de que o servidor está rodando.');
            console.error(err);
        } finally {
            setCarregando(false);
        }
    ...
    return (
        <DashboardLayout titulo="Consultar Paciente" subtitulo="Registos Clínicos">
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    {carregando ? (
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-50 text-slate-400 uppercase text-[10px] tracking-wider font-bold border-b border-slate-100">
                                        <th className="py-3 px-6">Paciente / Matrícula</th>
                                ...
                                <tbody className="divide-y divide-slate-100 text-sm">
                                    {pacientes.map((paciente) => (
                                        <tr key={paciente.matricula} className="hover:bg-slate-50/40 transition-colors">
                                            <td className="py-4 px-6">
                                                <div className="flex flex-col">
                                                    <span className="font-bold text-slate-800">{paciente.nome}</span>
                                                    <div className="flex items-center gap-3 text-xs text-slate-400 mt-0.5">
                                                        <span>Matrícula: <strong className="text-slate-600 font-mono">{paciente.matricula}</strong></span>
                                                        <span>•</span>
                                                        <span className="bg-blue-50 text-azulEscuro font-bold px-1.5 py-0.5 rounded text-[10px]">Sangue: {paciente.tipo_sanguineo || 'N/I'}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-4 px-6">
                                                <div className="flex flex-col">
                                                    <span className="font-medium text-slate-700">{paciente.cargo}</span>
                                                    <span className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                                                        <MapPin className="w-3 h-3" /> {paciente.setor}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="py-4 px-6 text-center">
                                                {/* Ao clicar, navega para a Ficha/Prontuário passando a matrícula na URL */}
                                                <button
                        ...
                        /* Empty State - Estado Vazio Tratado */
                        <div className="p-12 text-center max-w-sm mx-auto flex flex-col items-center">
                            ...
                            <h4 className="font-bold text-slate-700 text-base">Nenhum paciente localizado</h4>
                            <p className="text-xs text-slate-400 mt-1">Verifique os dados digitados ou realize um novo registo caso este funcionário esteja acessando o ambulatório pela primeira vez.</p>
                        </div>
                    )}
                </div>
